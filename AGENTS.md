...
- Open PRs against `main` by default. Stacked PRs may target another feature branch only when that exception is explicitly discussed for the current slice; once the parent branch lands, retarget/rebase the child PR back onto `main`.
