...
- Do not use public `news.md` bullets to list unchanged systems or guardrails such as "`/spirit`, group movement, combat, professions, Lisovyk remain unchanged." If a system did not change, omit it from public news. Put unchanged-scope guardrails in PR summaries, release-note risks, changelog risk notes, or technical docs only when they help reviewers.
- Do not use public `news.md` bullets just to list absent features such as no shops, no raw chance tables, no crafting screen, no combat rules or no public skill sheet. Mention limitations only when they directly prevent a likely player mistake in that same visible feature; otherwise keep them in release notes, changelog risks or technical docs.
...
- Open PRs against `main` by default. Do not infer a stacked PR base from the current local branch, commit ancestry, convenience, or wording like "after PR #123"; that wording is scheduling context, not permission to target the PR at another branch. A stacked PR may target another feature branch only when the user explicitly asks for a stacked PR or names the non-main base branch for the current slice. If an exception is used, state the stack in the PR title/body and final summary, and retarget/rebase the child PR onto `main` before presenting it as ready to merge.
...
- Treat runtime foundation PRs that are intended for merge as release-scoped by default, even when they are helper-only or not immediately player-facing. Give them a versioned branch/title/body and matching package, changelog, release-note and public-news surfaces unless the maintainer explicitly agrees in the current task that the PR is unversioned.
...
- On Windows PowerShell, prefer `npm.cmd test` / `npm.cmd run build` when `npm` is blocked by execution policy. The full `npm.cmd test` suite often exceeds 120 seconds as the manifest grows; the runner prints timings and supports `TEST_JOBS` for local speedups, for example `$env:TEST_JOBS=4; npm.cmd test`. Still start full-suite commands with a longer timeout, currently at least 300 seconds, and increase that baseline periodically as tests are added.
...
- When adding or renaming player-facing functionality, update the “Навчання” / tutorial-facing learning surface at least with short text hints when a full lesson is out of scope. Do not leave new commands discoverable only through `/help`, admin notes or release text.
...
- Player-facing slash command examples that need arguments or multi-word actions should also work with underscores in place of spaces, because Telegram clickable commands cannot include spaces. For example, `/attack_all_mouse` should parse like `/attack all mouse`, and `/sleep_tutorial` should parse like `/sleep tutorial`, unless a command has an explicitly documented exception.
- Do not add commands to the Telegram side command menu just because they appear in public news, Herald posts, `/help`, `/commands`, changelog or release notes. Keep the side menu intentionally short; update it only when the maintainer explicitly asks for a side-menu change. Public news/site/Herald command links should be backed by direct bot commands, safe `/start cmd_*` deep links and tests instead.
...
- When adding a new ordinary seeded starter animal species, add its starter groups through the shared starter-animal data so population-floor restoration and Дід Лісовик restoration can recover it by default. If a starter species should not be restorable, document the explicit exception and add a focused test.
