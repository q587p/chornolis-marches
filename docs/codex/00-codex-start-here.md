...
1c. Every numbered release must add a `news.md` entry. If the release is internal, scribe-only or operator-focused, write a short atmospheric public note without admin-only command names, hidden service tooling or implementation details; do not leave gaps in public news version numbers.
1c-1. Do not spend public `news.md` bullets on unchanged systems or guardrails. If `/spirit`, group movement, combat, professions, Lisovyk or another nearby system did not change, omit it from public news instead of saying it remains unchanged. Put that scope boundary in the PR body, changelog risks, release-note risks or technical docs when reviewers need it.
1c-2. Do not spend public `news.md` bullets only on absent features such as no shops, no raw chance tables, no crafting screens, no combat rules or no public skill sheet. Use that kind of limitation only when it directly prevents a likely player mistake in the same visible feature; otherwise keep it in release notes, changelog risks or technical docs.
1d. PRs target `main` by default. "After PR #123" is not permission to make a stacked PR; stack only when the user explicitly asks for a stacked PR or names the non-main base branch. Release-scoped PRs should show their version in the branch name and PR title/body, and should either include the matching docs/news/changelog/release notes or explicitly explain why the PR is unversioned/no-news.
1e. Runtime foundation PRs intended for merge are release-scoped by default, even when they are helper-only and not immediately player-facing. Use a versioned branch/title/body and matching package, changelog, release-note and public-news surfaces unless the maintainer explicitly agrees in the current task that the PR is unversioned.
...
12b-1. If a player-facing slash command example needs arguments or a multi-word action, its Telegram-friendly underscore form should work too: `/attack_all_mouse` should parse like `/attack all mouse`, `/sleep_tutorial` like `/sleep tutorial`, and `/queue_cancel` like `/queue cancel`, unless a command has an explicitly documented exception.
...
13a. When new player-facing functionality ships or gets renamed, check the repeatable “Навчання” / tutorial-facing learning surface too. If a full lesson would be too much for the current slice, add at least a short hint or planning item so follow, group, crafting/remedy and similar flows do not live only in release notes or `/help`.
...
15a. When adding a new ordinary seeded starter animal species, route its starter groups through the shared starter-animal data so population-floor restoration and Дід Лісовик restoration can recover it by default. If the species should not be restorable, document the explicit exception and cover it with a focused test.
...
- If that action belongs in player learning, update “Навчання” at least as text/hints, even when a complete guided lesson stays out of scope.
