...
12a. When a visible button/action appears in `/help`, `/commands`, news or release notes, write the stable slash command in parentheses after the label, e.g. `🌙 AFK / відійти` (`/afk`) or `🚪 Завершити сесію` (`/end_session`). Keep actual Telegram button labels clean: do not append slash-command hints inside reply/inline keyboard button text.
