...
- `start_cellar_root_pocket` intentionally uses `z = -1` as a tiny safe root pocket below the starter cellar. It is a MAP-004 light/examine proof, not a loot room: no ordinary resources, no combat and no broad route system. The moved `start_cellar_empty_shelf` is a no-loot herbalist staging marker there, not a container. The ALC-002 `start_root_pocket_bottle_niche` is the narrow authored exception: it provides one `empty_bottle` per take action for future herbalism while the player carries fewer than 3 empty bottles from that source path; it is not a general loot cache or practical unlimited stockpile.
- `start_cellar_root_gap` is an authored attention gate. Its hidden `INSIDE` edge exists for seed consistency, but the entry must stay out of ordinary visible exit lists until full feature inspection reveals the `Пролізти` (`/crawl`) action through the existing light/visibility rules.
...
- `meadow_16_05_grass_run` intentionally uses `z = -1` as a tiny low grass-run layer under the early dry meadow. It is the MAP-004 track/follow-memory proof: the hidden `INSIDE` entry from `meadow_16_05` should remain hidden, the visible return should stay `OUTSIDE`, and no ordinary resource nodes, loot cache, combat hook or auto-follow behavior should be added there.
