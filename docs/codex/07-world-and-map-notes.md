...
- `start_cellar_root_pocket` intentionally uses `z = -1` as a tiny safe root pocket below the starter cellar. It is a MAP-004 light/examine proof, not a loot room: no ordinary resources, no combat and no broad route system. The ALC-002 `start_root_pocket_bottle_niche` is the narrow authored exception: it provides one `empty_bottle` per take action for future herbalism, not a general loot cache.
