...
- `/archive_republish_gap` is intentionally read-only. Use it when `/archive_republish_status` says the current run has no pending rows, but deployed `news.md` may contain newer archive entries than the run originally queued.
