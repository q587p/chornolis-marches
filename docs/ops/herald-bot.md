...
publication_queue_status - показати повний стан скрині публікацій
future_publications - показати майбутні записи скрині публікацій
pending_publications - показати записи, готові до публікації зараз
...
archive_republish_preview - переглянути повну повторну архівну чергу
archive_republish_queue - поставити весь deployed архів у повторну чергу
archive_republish_status - звірити стан повторної архівної черги
archive_republish_gap - звірити deployed news.md із видимими архівними публікаціями
archive_republish_continue_preview - переглянути продовження пропущеного хвоста
archive_republish_continue - поставити пропущений хвіст архіву в чергу
archive_republish_cancel - скасувати неопубліковані записи повторної архівної черги
...
- `/publication_queue_status` — показати повний стан PUBLIC outbox: загальну кількість неопублікованих записів, готові зараз, майбутні, розподіл за `sourceType`, найближчий `availableAt` загалом і за типами джерела, а також pause/resume стан.
- `/future_publications` — показати наступні 10 неопублікованих PUBLIC записів із `availableAt` у майбутньому, включно з id, `sourceType`, назвою/`sourceId`, `archiveOrder` і київським часом доступності.
- `/pending_publications` — показати тільки записи, готові до публікації зараз (`availableAt <= now`), а не всю майбутню чергу.
...
- `/archive_republish_preview [30m]` — перечитати deployed `news.md` і показати повний план повторної архівної черги: кількість записів, перший/останній запис, інтервал, приблизне завершення та чи вже є активний queued run.
- `/archive_republish_queue [30m]` — поставити весь deployed архів у durable `HeraldPublication` чергу від найстарішого до найновішого. Без аргументу використовує 30 хвилин; некоректний інтервал дає видиму відповідь і нічого не ставить у чергу.
- `/archive_republish_status` — показати стан повторної архівної черги й наступний запис.
- `/archive_republish_gap` — read-only звірка deployed `news.md` із видимими опублікованими `NEWS_MD_ARCHIVE` / `NEWS_MD_ARCHIVE_REPUBLISH` записами. Показує загальну кількість deployed записів, скільки з них уже видно як опубліковані, скільки бракує, останній опублікований архівний індекс, перший пропущений індекс і до 20 пропущених записів. Якщо пропуски йдуть тільки після останнього опублікованого запису, це tail gap; якщо пропуски є перед уже опублікованими пізнішими записами, це internal holes. Команда нічого не публікує, не ставить у чергу й не змінює `HeraldPublication`.
- `/archive_republish_continue_preview [30m]` — перечитати deployed `news.md`, використати ту саму gap-звірку й показати тільки безпечний пропущений хвіст після останнього видимого архівного запису. Якщо є internal holes, команда відмовляє й просить ручну перевірку.
- `/archive_republish_continue [30m]` — поставити тільки пропущений tail gap у durable `HeraldPublication` чергу від найстарішого до найновішого. Інтервал обов’язковий; некоректний або порожній аргумент дає приклад. Команда не публікує одразу й не відновлює publisher loop.
- `/archive_republish_cancel` — скасувати тільки неопубліковані записи повторної архівної черги. Звичайні pending news/archive записи й уже опублікована історія не змінюються.
...
- `/archive_republish_queue` is intentionally separate from `/repost_publication`: it re-queues deployed `news.md` archive entries with source type `NEWS_MD_ARCHIVE_REPUBLISH`, stable `archiveOrder` / `availableAt` spacing and `republish:v1:*` content hashes. Channel posts use the normal archive header `📜 З архіву Канцелярії` and do not add a repeated-publication disclaimer.
- `/archive_republish_continue` uses the same source type and normal archive rendering, but its content hashes use `archive-continue:*` tied to the original archive entry. Re-running the command skips already existing continue rows instead of duplicating them.
- `/archive_republish_gap` is intentionally read-only. Use it when `/archive_republish_status` says the current run has no pending rows, but deployed `news.md` may contain newer archive entries than the run originally queued. If continue rows are queued but not yet published, the gap report still counts them as missing from the channel/book and also shows the queued continue count.
- `/archive_republish_cancel` marks only unpublished `NEWS_MD_ARCHIVE_REPUBLISH` rows as `CANCELED`; it does not touch normal pending news/archive rows or already published history.
...
5. Якщо канал налаштовано, перевірити `/publication_queue_status`, `/pending_publications` (готові зараз) і ручний `/publish_pending` на тестовому queued записі.
