...
- Deeper shrine and offering systems: after the first inspectable shrine/kapyshche feature exists, offerings can connect to calendar days, moon phases, dreams, spirits, weather, local safety, social memory and long-term consequences. Keep early offerings small and ambiguous; richer ritual logic belongs here until world time, inventory items and personal chronicle systems mature.
...
- Referral / invited-by system: think through how invitations work across Telegram-first play and a future ordinary MUD gateway. Telegram can use start parameters such as `?start=ref_116483297` / `/start ref_116483297` to attribute who invited a new player. A MUD client cannot rely on Telegram deep links, so onboarding may need an optional "who invited you?" prompt, a short referral code, or account/session-level invite metadata. Keep rewards and visibility subtle and abuse-resistant; this should support community growth without turning onboarding into a marketing flow.
...
- Large MUD-style socials catalog: use classic MUD social/emote lists such as https://www.continentsmud.com/socials.php as inspiration for a much broader future command set, but adapt it into Ukrainian, gender/case-aware, target-aware, observer-aware Чорноліс phrasing instead of copying English commands directly. Keep this behind the current small social-signal MVP until grammar, moderation, tutorial teaching and cross-client text input are stronger.
