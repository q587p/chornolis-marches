...
- `0.15.13` partial slice: the player-initiated feed affordance now exists through generic `give`, while autonomous meat-interest/theft scenes and cooldowns remain future work.
...
- `0.15.13` covers the player-carried raw-meat feed option and `/feed_raw_meat`/`/feed_raw_meet` alias-to-`give` routing. The remaining acceptance criteria below still describe the fuller theft/interest scene.
