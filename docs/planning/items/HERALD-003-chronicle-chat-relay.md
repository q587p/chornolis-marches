...
Watchpoint: the MVP dedupe uses `WorldEvent` relay markers. If old `WorldEvent`
rows are aggressively pruned, relay dedupe can forget prior sends. That is
acceptable for the first slice, but if chronicle relay becomes important or
high-volume, promote the marker into a durable relay/publication state table
instead of relying on retained event history.
