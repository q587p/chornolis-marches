...
- Add a first creation choice:
  ...
  - `Ввести власне ім’я`.
- Add a prepared-name source with unused, unreserved, scribe-approved names.
- Store Ukrainian case forms for prepared names.
- Show short origin/region, rarity and case-form availability for prepared names.
- Warn players before custom-name entry about disallowed names and tone mismatch.
- Validate custom names against:
  - exact existing player/NPC/significant-being names;
  - confusingly similar names;
  - creature/species names and important spirits;
...
- A new player can pick a suitable prepared name without waiting for manual review.
- A custom-name player sees clear guidance before entering a name.
