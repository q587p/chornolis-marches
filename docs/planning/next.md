...
- CAT-003: `0.15.11` adds the first pre-hunt camp-local mouse priority, so local mice make the cat watch before ordinary up/down movement. The next CAT-003 slice should add actual camp-local mouse hunting without letting the cat leave the starter camp or replace wilderness predators.
