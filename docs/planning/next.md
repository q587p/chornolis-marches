...
- ONB-009: high-priority tutorial dream cleanup. The dream already has a few unrelated ground items/resources drifting into it; clean, hide or origin-mark non-tutorial objects so new players only see authored lesson material there.
- WORLD-004: split the overloaded `/time` surface. Keep `/time` as a concise current-time/daypart check, move the full year/lunar-circle/calendar readout to a future `/calendar` or better-named equivalent, keep `/weather` as the sky/visibility surface, and update tutorial/help wording accordingly.
