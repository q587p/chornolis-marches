...
- GIVE-001/CAT-004: `0.15.11` adds the first generic `give` bridge for giving one raw-meat unit to the visible camp spirit cat, plus `Дати сире м'ясо`, `/feed_raw_meat` and `/feed_raw_meet` routing. Broader player/NPC transfer, barter, cooldowns and autonomous meat-theft scenes remain follow-up work.
- GIVE-002: `0.15.12` adds the first Ukrainian inflection-tolerant parser slice for the same raw-meat/camp-cat transfer: `дати м’яса коту`, `дати мясо кіт`, `дати сирого м’яса бережнику` and hyphenated `котові-бережнику` forms now resolve to the canonical `give raw meat cat` intent. Keep broader gifts, barter and item-instance ambiguity as follow-up work. Watchpoint: current `/give` is intentionally immediate; when `give` becomes a broader transfer/barter primitive, either queue it as an action or explicitly separate instant small gestures from time-costing item transfers.
...
- MONEY-001/LOOT-001: `0.15.16` adds the first small money and local low-risk loot MVP: `shah`/`grivna` resource names, starter-adjacent `shah` finds, pickup support, a separate beginner-cache money box and a rare waking-world gather bonus. `0.15.17` adds the first empty-search atmosphere for full `/examine`. Keep next loot work focused on authored small find objects, item lifetime and later offering/omen hooks; shops, vendors, prices, conversion, theft and broad economy remain out of scope.
...
- OBS-003/OWL-004: `0.15.17` adds danger-aware `/examine` atmosphere for raised local danger and the first conditional owl-sign examine echoes. Next observation work should move toward `LEARN-001`/`OBS-001` storage and observable learning moments rather than adding raw danger numbers.
- ONB-009: `0.15.18` hides unrelated ordinary ground items/resources from tutorial dream visible pickup/rendering paths while preserving authored berries/herbs for the foraging lesson. A later cleanup/origin-marking pass can delete or classify persisted leftovers if needed.
- WORLD-004: `0.15.18` splits the overloaded `/time` surface. `/time` is now a concise current-time/daypart check, `/calendar` carries year/lunar-circle/day/moon details, `/weather` remains the sky/visibility surface, and tutorial/help wording now teaches the split.
- APIARY-001/APIARY-002: `0.15.19` adds the first old log apiary / `Стара бортя` in the dry meadow and a rare passive bumblebee sting hazard around it. `0.15.20` starts `APIARY-003` with honey/beeswax resources and a risky, cooldown-limited hive robbery action. Keep honey uses, offerings/crafting/economy and the bear honey loop (`APIARY-004`) as follow-up work, not part of the first harvest slice.
...
- Local low-risk loot: `LOOT-001` has its first `MONEY-001` MVP slice after `0.15.16` and empty-search atmosphere after `0.15.17`. Review the next step as small authored find objects, item lifetime or offering hooks, not as a shop/economy expansion.
...
- Generic build command foundation: `BUILD-001` should turn `/build_campfire` into the first member of a reusable `build <thing>` family, so future shelters, gates and other construction targets do not become separate ad hoc parser branches.
- Targeted campfire commands: `FIRE-002` should let `/light_campfire_1`, `/douse_campfire_2`, `/dismantle_campfire_3` and matching text aliases choose a specific visible handmade campfire instead of relying on the current no-target first-match fallback.
