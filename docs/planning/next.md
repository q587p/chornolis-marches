...
- GIVE-002: `0.15.12` adds the first Ukrainian inflection-tolerant parser slice for the same raw-meat/camp-cat transfer: `дати м’яса коту`, `дати мясо кіт`, `дати сирого м’яса бережнику` and hyphenated `котові-бережнику` forms now resolve to the canonical `give raw meat cat` intent. Keep broader gifts, barter and item-instance ambiguity as follow-up work.
