...
- LOOP-002 / ECO-002: gate hunting loop follow-up, especially settlement response tuning and NPC hunter integration.
...
- Gate hunting loop: 0.13.7 adds the notice, падальний рів, narrow `put` flow and player contribution accounting. The next small slice should decide whether to tune settlement reactions first or implement `NPC-002` hunter behavior through the same drop-off service.
