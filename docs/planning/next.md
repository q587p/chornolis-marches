...
- GIVE-001/CAT-004: `0.15.13` adds the first generic `give` bridge for giving one raw-meat unit to the visible camp spirit cat, plus `Дати сире м'ясо`, `/feed_raw_meat` and `/feed_raw_meet` routing. Broader player/NPC transfer, barter, cooldowns and autonomous meat-theft scenes remain follow-up work.
