...
- `/who` — публічний список персонажів, активних за останню реальну годину; писарі показані окремо, інші гравці й NPC змішані в одному абетковому списку. Веб-версії `/who` і `/who.json` доступні всім без `ADMIN_SET_SECRET`.
...
- Веб-сторінка `/world` показує детальний службовий стан світу, чергу дій і останні події; як і `/all`, вона питає `ADMIN_SET_SECRET`. Публічна головна сторінка `/` показує український статус, версію, кількість активних у `/who`, посилання, емблему й короткий опис бачення та тону проєкту.
