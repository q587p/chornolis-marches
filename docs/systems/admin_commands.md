...
- `/playerAdmin <#id|ім’я|username>` — детальна службова картка гравця з будь-якої місцини: ім’я й відмінки, схвалення імені, стан, точні числа незалежно від локального debug-перемикача, роль, місцина, речі/ресурси, статистика, авто/ручні дії й останні дії. Картка має кнопки `Схвалити ім’я` / `Зняти схвалення` і перемикання авто для цього персонажа. Telegram `/all` також дає кнопки `👤 #id` для гравців на поточній сторінці.
