...
- NPC hunter behavior may take or refresh a bounded hunting bundle from the gate torch stand, currently planned as five unlit torches, before routing to a magic campfire to light the first one. A hunter returning from the field can also be seeded with a visible lit torch and a spare torch represented by the current lightweight hunter bundle marker. If the stand becomes limited stock, NPCs should use the same feature/container rules as players and avoid draining beginner-critical supply.
