...
- `/take_bottle`, `take bottle`, `взяти пляшечку`, `взяти пляшку` -> take one `порожня пляшечка` from a visible local `empty_bottle_source` feature such as the root-pocket bottle niche. If no source is present, the same target may fall back to ordinary visible ground pickup for dropped `empty_bottle` items.
- `/brew tincture`, `/brew herbal_tincture`, `/make_tincture`, `brew tincture`, `make tincture`, `prepare tincture`, `зробити настоянку`, `приготувати настоянку`, `зробити зілля`, `приготувати зілля` -> attempt to brew one `трав’яна настоянка` from a carried `empty_bottle`, `herbs` and `berries`.
- `/drink tincture`, `/drink herbal_tincture`, `/use tincture`, `/use herbal_tincture`, `drink tincture`, `use tincture`, `випити настоянку`, `випити зілля`, `використати настоянку` -> drink a carried `трав’яна настоянка` for a strong capped stamina restore and return one `порожня пляшечка`.
...
- `/pick_all`, `/get_all`, `get all`, `pick all`, `взяти все`, `підібрати все`, `підняти все` -> pick up all visible loose ground resources and visible corpses listed under `Лежить:` in the current місцина.
- `/pick_all corpse`, `/get_all corpse`, `get all corpse`, `get all mouse`, `get all berries`, `підняти всі трупи` -> pick up only matching visible things from `Лежить:` when the type can be resolved.
- Location pickup buttons may also show a compact `всі` action beside a loose resource stack; it picks up all visible loose resources of that same type. When any pickable ground objects are visible under `Лежить:`, `Підібрати все` uses the same all-visible pickup path as `/pick_all` / `get all`.
...
- Directed target speech to visible animals is allowed, but it is not human conversation. Mice and rabbits may flee or freeze, foxes may react warily, and repeated speech to a wolf can become a local danger warning rather than a reply.
