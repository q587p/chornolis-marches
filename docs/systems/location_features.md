...
- empty-bottle niche: a narrow starter-cellar/root-pocket utility feature with `data.empty_bottle_source = true`; `/examine` should explain that one `порожня пляшечка` can be taken for future herbal preparations. It is intentionally not a broad loot cache, shop, recipe UI or potion source.
