...
- Ambient spawns may appear in locations that currently have players, animals, profession NPCs, monsters or spirits. Presence alone is not protection: only region caps, existing active totems in the same location and protected/special regions exclude them. Current protected regions are the starter camp, old bridge, dream tutorial and closed settlement gate; ordinary riverbank locations such as the under-bridge passage are eligible when the usual caps and location guardrails allow it.
- Profession NPCs that understand the world well enough, currently hunters, знахарі and травниці, may queue a higher-priority local `DISMANTLE_TOTEM` action when they notice an active totem in their location. They use the same twig-recovery calculation as players, but the recovered `twigs` go into that creature's carried resources so later profession/fire behavior can use them.
