...
`0.15.26` adds the first lightweight visible herbalist route. A herbalist can occasionally stage through the starter cellar/watchtower, gather herbs, berries or mushrooms through the ordinary creature `GATHER_SPECIFIC` action path, then return to the cellar/root-pocket area to sort/rest narratively. The hidden root-pocket shelf remains a no-loot staging marker, not a shared container, and torch preparation is symbolic until NPC-held inventory/light is promoted.
