...
As of `0.14.11`, the watchtower also has a `Спільна скриня прибулих`: a small shared cache for simple first-road supplies such as berries, herbs, mushrooms, raw meat, cooked meat and twigs. Torches live on the nearby torch stand instead of inside the cache. The cache supports narrow take/contribute actions and hidden unobserved restock so the camp feels maintained without becoming a shop or reward table. Bulk contribution may queue one-at-a-time `Лишити всі/весь/все` actions for supplies the player is carrying, and `/put` can route supplies into the cache just as it routes carcasses into the dropoff pit. Taking remains one item at a time to keep the starter cache from becoming a fast-loot surface. Beginner copy should frame it as mutual care and practical preparation, not as free loot. For now, cache stock is stored in feature JSON and resets with seed/reset flows; future stewardship work can move it to a more durable item/stock layer and add anti-hoarding or contribution etiquette deliberately.
