...
- Tutorial and other explicitly dream-keyed/lucid-dream locations use stable dream light by default. Waking-world daypart and weather visibility reductions should not hide tutorial descriptions, visible lessons, tracks or ground objects inside the dream; otherwise a new player can miss the lesson just because the outside world is dark.
...
- As of `0.12.4`, the hub has a south branch:
  - `Плесо часу сну` now teaches three separate world-state surfaces through one landmark prompt: `Час` (`/time`) for the current daypart and approximate world time, `Погода` (`/weather`) for sky/visibility mood, and `Календар` (`/calendar`) for year, lunar circle, day and moon phase.
...
As of `0.14.11`, the watchtower also has a `Спільна скриня прибулих`: a small shared cache for simple first-road supplies such as berries, herbs, mushrooms, raw meat, cooked meat and twigs. Torches live on the nearby torch stand instead of inside the cache. As of `0.15.16`, the cache also has a separate small money box for `shah` and `grivna`; hidden unobserved restock must never create money, so coin stock changes only through seed/reset state or explicit player take/contribute actions. The cache supports narrow take/contribute actions and hidden unobserved restock so the camp feels maintained without becoming a shop or reward table. Bulk contribution may queue one-at-a-time `Лишити всі/весь/все` actions for supplies the player is carrying, and `/put` can route supplies into the cache just as it routes carcasses into the dropoff pit. Taking remains one item at a time to keep the starter cache from becoming a fast-loot surface. Beginner copy should frame it as mutual care and practical preparation, not as free loot. If Кіт-бережник is visibly near the cache, inspection may add one quiet presence line, but the cat does not own, guard, reward or block the supplies. A careful full examination of the cat may also vary with nearby mice, daypart, firelight or the watchtower. For now, cache stock is stored in feature JSON and resets with seed/reset flows; future stewardship work can move it to a more durable item/stock layer and add anti-hoarding or contribution etiquette deliberately.

After darkness starts hiding descriptions, beings, ground objects, resources or tracks, a waking-world character can receive one first-night guidance note when they are in dim/dark reduced visibility without local light. It must not trigger inside the tutorial dream, and it should remain behind the same presence/AFK guards used for other proactive guidance. The message should mention that fire or a torch can return part of what the dark hides, without listing raw visibility mechanics. Dream rooms keep their own teaching light unless a later special dream deliberately says otherwise.
