...
const base = {
  beginner_cache: true,
  cache_stock: { torch: 0, berries: 1, herbs: 0, mushrooms: 0, twigs: 1 },
  cache_max_stock: { torch: 3, berries: 6, herbs: 4, mushrooms: 4, twigs: 10 },
  cache_restock_target: { torch: 2, berries: 3, herbs: 1, mushrooms: 1, twigs: 5 },
...
assert.equal(beginnerCacheResourceKeyFromText("raw meat"), null);

assert.deepEqual(beginnerCacheTakeKeys(base), ["berries", "twigs"]);
...
assert.deepEqual(beginnerCacheStock(restocked), { torch: 2, berries: 3, herbs: 1, mushrooms: 1, twigs: 5 });
