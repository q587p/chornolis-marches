...
const pluralSummary = preparedNameCompactSummary(preparedNameByKey("berehovi"));
assert.equal(pluralSummary.includes("plural form"), false);
assert.ok(pluralSummary.includes("форма для «вони»"), "Expected plural prepared-name notes to be Ukrainian in player-facing summaries");
