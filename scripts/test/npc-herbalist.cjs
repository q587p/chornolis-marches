...
const learner = uniqueCreatures.find((creature) => creature.professionKey === NPC_LEARNER_PROFESSION_KEY);
...
assert.equal(learner.speciesKey, "local_person", "learner should not reuse the herbalist species archetype");
assert.equal(isHerbalistCreature({ species: { key: learner.speciesKey }, professionKey: learner.professionKey }), false);
