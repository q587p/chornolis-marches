...
assert.match(sittingActionBlockMessage("RAID_APIARY"), /борті по мед/);
...
assert.equal(adminMenuButtons.flat().includes("🐾 Види"), true);
...
assert.equal(adminMenuButtons.flat().includes("🎒 Додати речі"), true);
assert.equal(adminMenuButtons.flat().includes("🎒 Речі"), false);
