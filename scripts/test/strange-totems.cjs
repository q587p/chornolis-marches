...
const {
  ...
  STRANGE_TOTEM_PROTECTED_LOCATION_KEYS,
...
for (const locationKey of STRANGE_TOTEM_PROTECTED_LOCATION_KEYS) {
  const location = locations.find((candidate) => candidate.key === locationKey);
  assert.ok(location, `${locationKey} should exist in world data`);
  assert.equal(isStrangeTotemProtectedSpawnLocation({ key: location.key, region: { key: location.regionKey } }), true);
  assert.equal(staticEligibleLocations.some((eligible) => eligible.key === location.key), false);
}
