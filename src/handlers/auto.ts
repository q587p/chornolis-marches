...
async function autoGather(bot: Bot, telegramId: number, player: any, locationId: number) {
  ...
  if (!cfg || Math.random() > cfg.chance) {
    ...
    await bot.api.sendMessage(telegramId, `🤖 Авто: ви пошукали ${resource.resourceType.name}, але нічого корисного не знайшли.`, {
  ...
  await prisma.player.update({
    where: { id: player.id },
    data: { gatherAttempts: { increment: 1 }, successfulGathers: { increment: 1 } },
  });

  await bot.api.sendMessage(telegramId, `🤖 Авто: ви знайшли ${resource.resourceType.name} ×${found}.`, {
    reply_markup: buildMainReplyKeyboard(true),
  });
...
async function autoLook(bot: Bot, telegramId: number, player: any, locationId: number) {
  ...
  await bot.api.sendMessage(telegramId, `🤖 Авто: ви зупинилися й озирнулися.`, {
    reply_markup: buildMainReplyKeyboard(true),
  });
...
async function autoMove(bot: Bot, telegramId: number, player: any, locationId: number) {
  ...
  await notifyLocation(bot, locationId, player.id, "Хтось пішов звідси.");
  await prisma.player.update({
    where: { id: player.id },
    data: { currentLocationId: exit.toLocationId, steps: { increment: 1 } },
  });
  await notifyLocation(bot, exit.toLocationId, player.id, "Хтось прийшов сюди.");

  await bot.api.sendMessage(telegramId, `🤖 Авто: ви рушили — ${directionLabels[exit.direction]}.`, {
    reply_markup: buildMainReplyKeyboard(true),
  });
...
async function runAutoStep(bot: Bot, telegramId: number) {
  ...
  try {
    ...
    if (!player) {
      stopAuto(telegramId);
      await bot.api.sendMessage(telegramId, "Авто зупинено: персонажа не знайдено. Напиши /start.", {
