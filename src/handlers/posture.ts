...
export async function submitPosture(ctx: any, mode: PostureAliasMode) {
...
export function registerPostureHandlers(bot: Bot) {
  bot.command("sit", (ctx) => submitPosture(ctx, "sit"));
  bot.command("stand", (ctx) => submitPosture(ctx, "stand"));
  bot.hears(["Сісти", "Встати"], async (ctx) => {
    await submitPosture(ctx, ctx.message?.text === "Сісти" ? "sit" : "stand");
  });

  bot.callbackQuery("posture:sit", async (ctx) => {
    await safeAnswerCallbackQuery(ctx);
    await submitPosture(ctx, "sit");
  });

  bot.callbackQuery("posture:stand", async (ctx) => {
    await safeAnswerCallbackQuery(ctx);
    await submitPosture(ctx, "stand");
  });
}
