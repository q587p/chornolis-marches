...
const ADMIN_COMMANDS = [
  ...
  "/all dead — усі записи істот, включно з inactive/dead",
...
async function normalizeUniqueNpc(npc: UniqueNpcSpec) {
  ...
  if (!kept) {
    kept = await prisma.creature.create({ data: { speciesId: species.id, locationId: fallbackLocation.id, name: npc.name, hp: species.baseHp, isAlive: npc.alive, activity: npc.activity, currentAction: npc.currentAction } });
    created = true;
  }

  if (npc.speciesKey === "herbalist" && !kept.isAlive) {
    kept = await prisma.creature.update({ where: { id: kept.id }, data: { isAlive: true, locationId: fallbackLocation.id, hp: species.baseHp, activity: npc.activity, currentAction: npc.currentAction } });
  }

  if (npc.speciesKey === "lisovyk" && kept.hp <= 0) {
    kept = await prisma.creature.update({ where: { id: kept.id }, data: { hp: species.baseHp } });
  }
...
export function registerStatusHandlers(bot: Bot) {
  ...
  bot.command(["addCreatureHelp", "addcreaturehelp"], async (ctx) => {
    ...
    const lines = species.map((s) => `${s.key} — ${s.name}; HP=${s.baseHp}; diet=${s.diet}; STR=${s.strength}; AGI=${s.agility}; PER=${s.perception}`);
  ...
  bot.command("all", async (ctx) => {
    ...
    const [players, creatures] = await Promise.all([
      ...
      prisma.creature.findMany({ where: showDead ? undefined : { isAlive: true }, include: { location: true, species: true }, orderBy: { id: "asc" } }),
    ]);
  ...
  bot.command(["cleanupCreatures", "cleanupcreatures"], async (ctx) => {
    ...
    const deadCleanup = await prisma.creature.deleteMany({ where: { isAlive: false, id: keepIds.length ? { notIn: keepIds } : undefined } });
    const duplicateCount = uniqueResults.reduce((sum, r) => sum + r.removed, 0);
    await prisma.worldEvent.create({ data: { type: "SYSTEM", title: "Creature cleanup", description: `Removed animals=${animalsRemoved.count}; removed dead=${deadCleanup.count}; unique NPC duplicates=${duplicateCount}.` } });
  ...
  bot.command(["cleanupCreature", "cleanupcreature"], async (ctx) => {
    ...
    const creature = await prisma.creature.findFirst({
      where: { isAlive: true, locationId: player.currentLocationId, species: { kind: "ANIMAL", ...(speciesKey ? { key: speciesKey } : {}) } },
  ...
  bot.command(["addCreature", "addcreature"], async (ctx) => {
    ...
    if (!speciesKey || !locationKey || !Number.isFinite(count)) return void (await ctx.reply("⚠️ Формат: /addCreature <speciesKey> <locationKey> [count]\nНаприклад: /addCreature rabbit center_chornolis_edge 3"));
    ...
    for (let i = 0; i < count; i++) await prisma.creature.create({ data: { speciesId: species.id, locationId: location.id, hp: species.baseHp, isAlive: true, activity: "IDLE", currentAction: "прислухається" } });
    await prisma.worldEvent.create({ data: { type: "SYSTEM", title: "Creature added", description: `Added ${speciesKey} ×${count} to ${locationKey}.`, locationId: location.id } });
