...
function parseFeatureInspectionIntent(text: string): ParsedAliasCommand | null {
  const match = text.match(/^(?:look\s+at|look|x|examine|inspect|роздивитися|роздивитись|придивитися\s+до|придивитись\s+до|придивитися|придивитись|оглянути|огл|глянути\s+на|подивитися\s+на)\s+(.+)$/);
