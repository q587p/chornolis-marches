...
type MovePayload = { direction: Direction };
...
function actionTitle(action: Pick<WorldAction, "type" | "payload" | "durationMs">) {
  if (action.type === "MOVE") {
    ...
    return `йдемо на ${directionLabels[payload.direction].toLowerCase()}`;
  }
...
export function movementDurationMs(travelCost = 1) {
  return Math.max(MIN_ACTION_DURATION_MS, DEFAULT_ACTION_DURATION_MS * Math.max(1, travelCost));
}

export function gatherDurationMs(resourceKey: keyof typeof gatherConfig) {
  const cfg = gatherConfig[resourceKey];
  if (!cfg) return DEFAULT_ACTION_DURATION_MS;
  return Math.max(MIN_ACTION_DURATION_MS, cfg.ticks * TICK_MS);
}

export function actionDurationMs(type: WorldActionType) {
  return Math.max(MIN_ACTION_DURATION_MS, actionDurationConfig[type] ?? DEFAULT_ACTION_DURATION_MS);
}
...
export async function enqueuePlayerAction(input: {
  ...
  return enqueueWorldAction({ ...input, actorType: "PLAYER" });
}
...
export async function renderPlayerActionQueue(playerId: number) {
  ...
  if (!actions.length) return "Черга дій порожня.";
...
export async function cancelCurrentPlayerAction(playerId: number) {
  return prisma.worldAction.updateMany({
    where: { actorType: "PLAYER", playerId, status: "RUNNING" },
    data: { status: "CANCELLED" },
  });
}
...
async function startNextQueuedAction(action: WorldAction) {
  await prisma.worldAction.update({
    ...
    data: {
      status: "RUNNING",
      startedAt: new Date(),
      executeAt: new Date(Date.now() + action.durationMs),
    },
  });
...
async function completeMove(bot: Bot, action: WorldAction) {
  ...
  await prisma.creature.update({ where: { id: creature.id }, data: { locationId: exit.toLocationId, activity: "MOVING", currentAction: actionTitle(action), steps: { increment: 1 }, hunger: { increment: 1 } } });
...
async function completeGather(bot: Bot, action: WorldAction) {
  ...
  if (isPlayer) await prisma.player.update({ where: { id: (actor as any).id }, data: { gatherAttempts: { increment: 1 } } });
  else await prisma.creature.update({ where: { id: (actor as any).id }, data: { gatherAttempts: { increment: 1 }, activity: "GATHERING", currentAction: `збирає ${payload.resourceKey}` } });
...
async function completeEat(action: WorldAction) {
  ...
  if (resource && creature.species.diet !== "CARNIVORE") {
    ...
    await prisma.creature.update({ where: { id: creature.id }, data: { hunger: Math.max(0, creature.hunger - 3), activity: "RESTING", currentAction: `їсть ${resource.resourceType.name}` } });
  } else {
...
async function completeAttack(bot: Bot, action: WorldAction) {
  ...
  await prisma.worldAction.updateMany({ where: { actorType: "CREATURE", creatureId: creature.id, status: { in: ["QUEUED", "RUNNING"] } }, data: { status: "CANCELLED" } });
...
async function completeTrack(bot: Bot, action: WorldAction) {
  const chatId = chatIdFromAction(action);
  await prisma.worldAction.update({ where: { id: action.id }, data: { status: "DONE" } });
  if (chatId) await bot.api.sendMessage(chatId, "👣 Ви вдивляєтесь у сліди. Поки що відслідковування ще не реалізоване, але напрямок втечі відчутний у землі й траві.");
}

async function completeSimple(action: WorldAction) {
  if (action.actorType === "CREATURE" && action.creatureId) await prisma.creature.update({ where: { id: action.creatureId }, data: { activity: "RESTING", currentAction: actionTitle(action) } });
  await prisma.worldAction.update({ where: { id: action.id }, data: { status: "DONE" } });
}
