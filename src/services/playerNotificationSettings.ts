...
export const AUTO_ACTION_MESSAGES_ENABLED_TEXT =
  "Повідомлення Поклику увімкнено. Якщо Поклик духа веде персонажа далі, я показуватиму вибір і результати його дій навіть після довшої тиші з вашого боку.";

export const AUTO_ACTION_MESSAGES_DISABLED_TEXT =
  "Повідомлення Поклику вимкнено. Поклик духа може вести персонажа далі, але після вашої довшої тиші Порубіжжя не засипатиме чат кожним кроком.";
...
export function renderNotificationSettings(input: PlayerNotificationSettings) {
  return [
    ...
    `Повідомлення Поклику: ${input.autoActionMessagesEnabled ? "увімкнено" : "вимкнено"}`,
    "",
    ...
    input.autoActionMessagesEnabled
      ? "Повідомлення Поклику показують, що саме обирає тихий поклик і чим закінчуються його дії, навіть якщо ви вже давно не натискали кнопок."
      : "Коли Поклик духа озивається, але ви довго не взаємодієте з ботом, частина рутинних повідомлень може стихати, щоб не перетворюватися на флуд.",
  ].join("\n");
}
...
export function buildSettingsKeyboard(input: PlayerNotificationSettings) {
  ...
  if (input.autoActionMessagesEnabled) {
    keyboard.text("Вимкнути повідомлення Поклику", "settings:autoMessages:off");
  } else {
    keyboard.text("Увімкнути повідомлення Поклику", "settings:autoMessages:on");
  }
